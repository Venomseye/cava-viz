...
bool Config::load() {
  ...
  while (std::fgets(line, sizeof(line), f)) {
    ...
    if (!strcmp(k, "theme"))
      ...
      hud_pinned = asBool(v);
    // Rendering modes
    else if (!strcmp(k, "outline_mode"))
      outline_mode = asBool(v);
...
void Config::save() const {
  ...
  fprintf(f, "\n# ── Rendering modes "
             "──────────────────────────────────────────────\n");
  fprintf(
      f, "# outline_mode: draw only the top cell of each bar (lighter look)\n");
  fprintf(f, "outline_mode   = %d\n", outline_mode ? 1 : 0);
  fprintf(f, "# colour_cycle: slowly rotate gradient hue over time\n");
  fprintf(f, "colour_cycle   = %d\n", colour_cycle ? 1 : 0);
  fprintf(
      f,
      "# per_bar_colour: map colour to bar index (bass=dark, treble=bright)\n");
