...
// ── Frame timing
// ────────────────────────────────────────────────────────────── Use
// clock_nanosleep (absolute deadline) on Linux for low drift. Fall back to
// std::this_thread::sleep_for on other platforms.
...
static void sleepUntil(struct timespec &deadline, long frame_ns) {
  ...
  const long long now_ns =
      (long long)now.tv_sec * 1'000'000'000LL + now.tv_nsec;
  const long long dead_ns =
      (long long)deadline.tv_sec * 1'000'000'000LL + deadline.tv_nsec;
  if (dead_ns < now_ns) {
...
int runBarMode(const std::string &backend, const std::string &cli_source,
  ...
  std::fprintf(stderr,
               "cava-viz: bar mode  source=%s  backend=%s  "
               "format=%d  fps=%d  count=%d\n",
               active_source.empty() ? "(default)" : active_source.c_str(),
               bname.c_str(), (int)bar_cfg.format, bar_cfg.fps, bar_cfg.count);
  ...
  while (running.load()) {
    ...
    const bool all_silent =
        std::all_of(bl.begin(), bl.end(), [](float v) { return v < 1e-4f; });
    ...
    if (silent_frames > reconnect_frames) {
      ...
      audio->stop();
      audio.reset();
      doStartAudio(backend, cli_source, use_mic, sample_rate, channels, fft,
                   cfg, audio, active_source, bname);
      if (!audio) {
    ...
    if (!writer.write(frame)) {
      ...
      if (writer.sinkType() == BarSink::Fifo) {
        ...
        if (!writer.reopen()) {
          std::fprintf(stderr, "cava-viz: failed to reopen FIFO, exiting.\n");
          break;
        }
      } else {
