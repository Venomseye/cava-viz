...
static void print_usage(const char *p) {
  printf("Usage: %s [OPTIONS]\n\n"
         ...
         "  --bar                      Enable bar mode\n"
         "  --bar-format <fmt>         "
         "plain|waybar|polybar|eww|raw|dzen2|i3bar  (default: plain)\n"
         "  --bar-count  <n>           Bars per channel  (default: 10)\n"
         "  --bar-chars  <chars>       UTF-8 level characters  (default: "
         "▁▂▃▄▅▆▇█)\n"
         "  --bar-color  <#hex>        Accent color for tagged formats  "
         "(default: #00ffcc)\n"
...
int main(int argc, char *argv[]) {
  ...
  while ((o = getopt_long(argc, argv, "b:s:Mr:t:f:wVh", long_opts, nullptr)) !=
    ...
    case 'r':
      ...
      break;
    case 't': {
      int ti = std::stoi(optarg);
      if (ti >= 0 && ti < (int)Theme::COUNT)
        cfg.theme = ti;
    } break;
    case 'f':
      cfg.fps = std::max(1, std::stoi(optarg));
      break;
    case 'w':
    ...
    case OPT_BAR_FORMAT: {
      std::string f = optarg;
      if (f == "plain")
        bar_cfg.format = BarFormat::Plain;
      else if (f == "waybar")
        bar_cfg.format = BarFormat::Waybar;
      else if (f == "polybar")
        bar_cfg.format = BarFormat::Polybar;
      else if (f == "eww")
        bar_cfg.format = BarFormat::Eww;
      else if (f == "raw")
        bar_cfg.format = BarFormat::Raw;
      else if (f == "dzen2")
        bar_cfg.format = BarFormat::Dzen2;
      else if (f == "i3bar")
        bar_cfg.format = BarFormat::I3bar;
      else {
        fprintf(stderr, "cava-viz: unknown --bar-format '%s'\n", optarg);
        return 1;
      }
      break;
    }
    case OPT_BAR_COUNT:
      bar_cfg.count = std::max(1, std::stoi(optarg));
      break;
    case OPT_BAR_CHARS:
      bar_cfg.chars = optarg;
      break;
    case OPT_BAR_COLOR:
      bar_cfg.color = optarg;
      break;
    case OPT_BAR_FPS:
      bar_cfg.fps = std::max(1, std::stoi(optarg));
      break;
    case OPT_BAR_STEREO: {
      std::string s = optarg;
      if (s == "merge")
        bar_cfg.stereo = BarStereo::Merge;
      else if (s == "split")
        bar_cfg.stereo = BarStereo::Split;
      else {
        fprintf(stderr, "cava-viz: unknown --bar-stereo '%s'\n", optarg);
        return 1;
      }
      break;
    }
    case OPT_BAR_SEP:
      bar_cfg.stereo_sep = optarg;
      break;
    case OPT_BAR_SINK: {
      std::string s = optarg;
      if (s == "stdout")
        bar_cfg.sink = BarSink::Stdout;
      else if (s == "fifo")
        bar_cfg.sink = BarSink::Fifo;
      else if (s == "socket")
        bar_cfg.sink = BarSink::Socket;
      else {
        fprintf(stderr, "cava-viz: unknown --bar-sink '%s'\n", optarg);
        return 1;
      }
      break;
    }
    case OPT_BAR_OUT:
      bar_cfg.sink_path = optarg;
      break;
    default:
      print_usage(argv[0]);
      return 1;
  ...
  if (bar_mode) {
    ...
    return runBarMode(backend, cli_source, use_mic, sample_rate, cfg, bar_cfg,
                      g_running);
  }
      ...
      while ((ilen = read(inotify_fd, ibuf, sizeof(ibuf))) > 0) {
        for (const char *p = ibuf; p < ibuf + ilen;) {
          ...
          if (ev->wd == wd_cfg && ev->len > 0 && cfg_file == ev->name) {
            ...
            const bool is_theme =
                (ev->len > 6) && std::string(ev->name).rfind(".theme") ==
