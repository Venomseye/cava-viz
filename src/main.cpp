...
// ── Audio factory
// ─────────────────────────────────────────────────────────────
static std::unique_ptr<AudioCapture> makeAudio(const std::string &backend,
  ...
  (void)sr;
  (void)ch;
  (void)cb; // suppress warnings when both disabled
  return a;
}
...
int main(int argc, char *argv[]) {
  ...
  static const struct option long_opts[] = {
      ...
      {nullptr, 0, nullptr, 0}};
  int o;
  while ((o = getopt_long(argc, argv, "b:s:Mr:t:f:wh", long_opts, nullptr)) !=
         -1) {
    switch (o) {
    case 'b':
      backend = optarg;
  ...
  auto startAudio = [&]() {
    if (!cli_source.empty()) {
      ...
      audio =
          makeAudio(backend, active_source, sample_rate, channels, make_cb());
    } else if (use_mic) {
      active_source = "";
      audio =
          makeAudio(backend, active_source, sample_rate, channels, make_cb());
      ...
      if (!audio) {
        ...
        if (!active_source.empty())
          audio = makeAudio(backend, active_source, sample_rate, channels,
                            make_cb());
      }
      if (!audio) {
        active_source = "";
