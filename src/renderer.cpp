...
static const Stop kGrad[static_cast<int>(Theme::COUNT)][7] = {
    ...
    {{{0.00f},{1000,450,  0}}, {{0.20f},{1000,250,  0}},
     {{0.40f},{ 980, 70,  0}}, {{0.56f},{ 940,  0,120}},
     {{0.72f},{ 880,  0,300}}, {{0.86f},{ 820,  0,450}},
     {{1.00f},{ 760,  0,580}}},

    // PLASMA  electric-violet → deep-blue → electric-blue → neon-cyan
    {{{0.00f},{ 480,  0,880}}, {{0.18f},{ 300,  0,960}},
     {{0.36f},{ 100,  0,1000}},{{0.54f},{   0,120,1000}},
     {{0.70f},{   0,500,1000}},{{0.84f},{   0,800,1000}},
     {{1.00f},{   0,980, 900}}},

    // NEON  deep-magenta → hot-pink → bright-lavender
    {{{0.00f},{ 920,  0,700}}, {{0.18f},{ 980,  0,820}},
     {{0.36f},{1000, 60,860}}, {{0.54f},{1000,180,800}},
     {{0.70f},{ 980,360,920}}, {{0.84f},{ 900,400,1000}},
     {{1.00f},{ 800,480,1000}}},

    // TEAL  deep-teal → teal → bright-aqua → pale-aqua
    {{{0.00f},{   0,680,660}}, {{0.18f},{   0,760,740}},
     {{0.36f},{   0,840,820}}, {{0.54f},{  20,900,880}},
     {{0.70f},{  60,960,940}}, {{0.84f},{ 140,990,970}},
     {{1.00f},{ 300,1000,980}}},

    // SUNSET  amber → orange-red → crimson → violet
    {{{0.00f},{1000,720,  0}}, {{0.18f},{1000,500,  0}},
     {{0.36f},{1000,280,  0}}, {{0.54f},{ 980, 60,  0}},
     {{0.70f},{ 900,  0,200}}, {{0.84f},{ 700,  0,420}},
     {{1.00f},{ 480,  0,640}}},

    // CANDY  hot-pink → magenta-violet → electric-blue → bright-teal
    {{{0.00f},{ 980,120,700}}, {{0.18f},{ 900, 20,800}},
     {{0.36f},{ 700,  0,900}}, {{0.54f},{ 400,  0,980}},
     {{0.70f},{ 100, 80,1000}},{{0.84f},{   0,480,1000}},
     {{1.00f},{   0,800, 960}}},

    // AURORA  vivid-green → teal-blue → violet → bright-lavender
    {{{0.00f},{   0,820,320}}, {{0.18f},{   0,720,520}},
     {{0.36f},{   0,560,720}}, {{0.54f},{ 100,240,880}},
     {{0.70f},{ 380, 50,900}}, {{0.84f},{ 600,  0,920}},
     {{1.00f},{ 760, 80,960}}},

    // INFERNO  deep-red → red-orange → amber → gold
    {{{0.00f},{ 820,  0,  0}}, {{0.18f},{ 920, 60,  0}},
     {{0.36f},{ 990,200,  0}}, {{0.54f},{1000,420,  0}},
     {{0.70f},{1000,680,  0}}, {{0.84f},{1000,860, 80}},
     {{1.00f},{1000,980,400}}},

    // WHITE  cool-slate → silver → pearl → pure-white
    {{{0.00f},{ 400,420,480}}, {{0.18f},{ 520,540,600}},
     {{0.36f},{ 640,660,720}}, {{0.54f},{ 760,780,840}},
     {{0.70f},{ 860,878,930}}, {{0.84f},{ 940,950,980}},
     {{1.00f},{1000,1000,1000}}},

    // ROSE  deep-rose → coral → salmon → blush
    {{{0.00f},{ 920,100,280}}, {{0.18f},{ 960,160,340}},
     {{0.36f},{ 990,240,420}}, {{0.54f},{1000,360,540}},
     {{0.70f},{1000,500,660}}, {{0.84f},{1000,660,780}},
     {{1.00f},{1000,800,900}}},

    // MERMAID  ocean-blue → seafoam → periwinkle → orchid
    {{{0.00f},{   0,380,800}}, {{0.18f},{   0,540,700}},
     {{0.36f},{   0,680,620}}, {{0.54f},{ 100,760,580}},
     {{0.70f},{ 400,420,800}}, {{0.84f},{ 600,200,900}},
     {{1.00f},{ 800,100,900}}},

    // VAPOR (synthwave)  neon-purple → magenta → hot-pink → neon-cyan
    {{{0.00f},{ 600,  0,820}}, {{0.18f},{ 800,  0,720}},
     {{0.36f},{1000,  0,600}}, {{0.54f},{1000,100,480}},
     {{0.70f},{ 820,120,900}}, {{0.84f},{ 200,600,1000}},
     {{1.00f},{   0,900,1000}}},
};
...
static RGB hsvToRgb(float h, float s, float v) noexcept {
    ...
    const int   i  = (int)hh;
    const float ff = hh - i, p = v*(1-s), q = v*(1-s*ff), t_ = v*(1-s*(1-ff));
    float r=0,g=0,b=0;
    switch(i) {
...
// ── Truecolor detection ───────────────────────────────────────────────────────
// Most modern terminals support init_color() but TERM=xterm-256color makes
// can_change_color() return false (the terminfo entry omits "ccc"). Detect
// by the environment variables terminals reliably set instead.
...
void Renderer::rebuildColors() {
    ...
    if (can_rgb_) {
        ...
        for (int i = 0; i < grad_steps_; ++i) {
            ...
            const short fg = f < 0.40f ? COLOR_YELLOW : f < 0.72f ? COLOR_RED : COLOR_WHITE;
            init_pair(i + 1, fg, -1);
        }
    }

    // ── HUD accent colours ────────────────────────────────────────────────────
    // Placed at color indices COLOR_BASE + grad_steps_ .. COLOR_BASE + grad_steps_ + HUD_PAIRS - 1
    // and ncurses pairs      grad_steps_ + 1           .. grad_steps_ + HUD_PAIRS
    // Both ranges are always within valid bounds (verified above).
    static const float kHF[HUD_PAIRS] = {0.55f, 0.72f, 0.88f, 1.00f};
    for (int lv = 0; lv < HUD_PAIRS; ++lv) {
        const short hud_ci = (short)(COLOR_BASE + grad_steps_ + lv);  // within [0, COLORS-1] ✓
        if (can_rgb_) {
    ...
    for (int line = 0; line < max_line; ++line) {
        ...
        if (outline_mode_) {
            if (line == cur_lh && height_sub > 0) {
                ...
                lbuf[bar_w_ * 3] = '\0';
                attron(attr); mvaddstr(row, col, lbuf); attroff(attr);
...
void Renderer::render(const std::vector<float>& bars_l,
    ...
    bool do_clear = needs_clear_;
    {
        ...
        if (beat_now != beat_flash_) { do_clear = true; invalidatePrev(); }
