...
bool App::startAudio() {
  ...
  // Called from the audio capture thread — FFTProcessor::addSamples is
  // mutex-safe.
  auto cb = [this](const std::vector<float> &samples, int ch) {
    if (fft_)
      fft_->addSamples(samples, ch);
  };

  auto tryBackend = [&](std::unique_ptr<AudioCapture> cap) -> bool {
    if (!cap->init(src, sample_rate_, channels_))
      return false;
    if (!cap->start(cb))
      return false;
...
bool App::handleInput(int ch, int /*target_fps*/) {
  ...
  case 'T': {
    int prev =
        ((int)renderer_.theme() - 1 + (int)Theme::COUNT) % (int)Theme::COUNT;
  ...
  case '=':
    ...
    renderer_.showFeedback("Sens: " +
                           std::to_string(cfg_.sensitivity).substr(0, 4));
    cfg_.save();
    break;
  case '-':
    cfg_.auto_sens = false;
...
int App::run(int argc, char *argv[]) {
  ...
  while (g_running.load(std::memory_order_relaxed)) {
    ...
    renderer_.render(bars_l, bars_r, fps_, backend_name_, cfg_.sensitivity,
                     cfg_.auto_sens);
    ...
    if (fps_tick - fps_ref >= std::chrono::seconds(1)) {
      fps_ = fcount / std::chrono::duration<double>(fps_tick - fps_ref).count();
      fcount = 0;
      fps_ref = fps_tick;
    }
  }
