...
static std::string buildBarStr(const std::vector<float>& bars,
    ...
    for (float v : bars) s += mapChar(v, chars);
    return s;
}
...
{
    if (bars_l.empty()) return "";

    const std::vector<std::string> chars = splitUtf8(cfg.chars);
    if (chars.size() < 2) return "";  // need at least 2 levels
    ...
    if (cfg.format == BarFormat::Raw) {
        ...
        if (split) {
            ...
            emit(cfg.stereo == BarStereo::Merge && !bars_r.empty()
                 ? mergeStereo(bars_l, bars_r) : bars_l);
        }
    ...
    switch (cfg.format) {
        ...
        case BarFormat::Waybar:
            return "{\"text\":\"<span color='" + col + "'>"
                   + text + "</span>\",\"class\":\"viz-active\"}";
...
bool BarWriter::write(const std::string& frame) {
    ...
    switch (sink_type_) {
...
#ifdef __linux__
        case BarSink::Fifo: {
            if (fifo_fd_ < 0) return false;
            const ssize_t n = ::write(fifo_fd_, line.data(), (ssize_t)line.size());
            if (n >= 0) return true;
            if (errno == EAGAIN || errno == EWOULDBLOCK) return true; // reader slow, skip frame
        ...
        case BarSink::Socket: {
            acceptClients();
            if (clients_.empty()) return true;  // no readers yet — not an error
...
bool BarWriter::reopen() {
    ...
    if (sink_type_ == BarSink::Fifo) {
        if (fifo_fd_ >= 0) { ::close(fifo_fd_); fifo_fd_ = -1; }
        return openFifo();
    }
#endif
    return false;
...
bool BarWriter::openFifo() {
    ...
    // This is intentional: the program waits for a status bar / script to connect.
...
bool BarWriter::openSocket() {
    if (path_.empty()) {
        std::fprintf(stderr, "cava-viz: --bar-out path required for socket sink.\n");
        return false;
    }
